...
int sceKernelGetModuleInfoByName(const char* name, OrbisKernelModuleInfo* info)
{
    ...
    if (ret) {
        klog("sceKernelGetModuleList (%X)", ret);
        return ret;
    }
    ...
    for (size_t i = 0; i < numModules; ++i)
    {
        ...
        if (ret) {
            klog("_sceKernelGetModuleInfo (%X)", ret);
            return ret;
        }
...
int get_module_base(const char* name, uint64_t* base, uint64_t* size)
{
    ...
    if (ret)
    {
        klog("[!] sceKernelGetModuleInfoByName('%s') failed: 0x%08X", name, ret);
        return 0;
    }
...
int patch_module(const char* name, module_patch_cb_t* patch_cb, void* arg)
{
    ...
    if (!get_module_base(name, &base, &size)) {
        klog("[!] get_module_base return error");
        return 0;
    }
    klog("[i] '%s' module base=0x%lX size=%ld", name, base, size);

    ret = sceKernelMprotect((void*)base, size, ORBIS_KERNEL_PROT_CPU_ALL);
    if (ret) {
        klog("[!] sceKernelMprotect(%lX) failed: 0x%08X", base, ret);
        return 0;
    }

    klog("[+] patching module '%s'...", name);
    if (patch_cb)
...
static void sdmemory_patches_cb(void* patches, uint8_t* base, uint64_t size)
{
    for (const orbis_patch_t *patch = patches; patch->size > 0; patch++)
    {
        ...
        klog("(W) ptr=%p : %08X (%d bytes)", base, patch->offset, patch->size);
...
int get_firmware_version(void)
{
    ...
    {
        klog("Error: can't detect firmware version!");
        return (-1);
    }

    klog("[i] PS4 Firmware %X", fw >> 16);
    return (fw >> 16);
}
...
int find_process_pid(const char* proc_name, int* pid)
{
    ...
    for (size_t i = 0; i < pnum; i++)
        ...
        {
            klog("[i] Found '%s' PID %d", proc_name, proc_list[i].pid);
            *pid = proc_list[i].pid;
            free(proc_list);
            return 1;
        }

    klog("[!] '%s' Not Found", proc_name);
    free(proc_list);
    return 0;
}
...
int find_map_entry_start(int pid, const char* entry_name, uint64_t* start)
{
    ...
    klog("Printing proc_vm_map_args.maps:");
    for (size_t i = 0; i < proc_vm_map_args.num; ++i) {
        klog("maps[%zu]: %p", i, proc_vm_map_args.maps[i]);
    }
    ...
    for (size_t i = 0; i < proc_vm_map_args.num; i++)
        ...
        {
            klog("Found '%s' Start addr %lX", entry_name, proc_vm_map_args.maps[i].start);
            *start = proc_vm_map_args.maps[i].start;
            free(proc_vm_map_args.maps);
            return 1;
        }

    klog("[!] '%s' Not Found", entry_name);
    free(proc_vm_map_args.maps);
    return 0;
}
...
static void write_SceShellCore(int pid, uint64_t start, const orbis_patch_t* patches)
{
    ...
    for (const orbis_patch_t *patch = patches; patch->size > 0; patch++)
    {
        ...
        klog("(W) %lX : %08X (size %d)", start, patch->offset, patch->size);
        dump_data((uint8_t*) patch->data, patch->size);
    }
    free(data);
}
...
int unpatch_SceShellCore(void)
{
    ...
    {
        klog("Error: Unable to unpatch SceShellCore");
        return 0;
    }
...
static int patch_SceShellCore(const orbis_patch_t* shellcore_patch)
{
    ...
    {
        klog("Error: Unable to patch SceShellCore");
        return 0;
    }
    ...
    if (!shellcore_backup)
    {
        ...
        for (i = 0; shellcore_patch[i].size > 0; i++)
        {
            ...
            klog("(R) %lX : %08X (%d bytes)", ex_start, shellcore_backup[i].offset, shellcore_backup[i].size);
...
restart:;
    ...
    for(;;)
    {
        ...
        if(strncmp(p_comm, proc_name, sizeof(p_comm)) == 0)
        {
            ...
            klog("Found '%s' PID (%d) %lX", p_comm, pid, entry.next);
            *proc_pid = pid;
            break;
        }
    ...
    do
    {
        ...
        {
            klog("Found '%s' start (%lX)", entry.name, entry.start);
