...
void orbisPadFinish()
{
	...
	{
		klog("scePadClose return 0x%8X\n", ret);
	}
	orbispad_initialized=0;

	klog("ORBISPAD finished\n");
}
...
int orbisPadInit(void)
{
	...
	{
		klog("ORBISPAD already initialized!");
		return orbispad_initialized;
	}
	...
	{
		klog("sceUserServiceInitialize Error 0x%8X", ret);
		return -1;
	}
	...
	if(ret<0)
	{
		klog("scePadInit Error 0x%8X", ret);
		return -1;
	}
	...
	if(ret<0)
	{
		klog("sceUserServiceGetInitialUser Error 0x%8X", ret);
		return -1;
//		orbisPadConf.userId=0x10000000;
	...
	{
		klog("scePadOpen Error 0x%8X", orbisPadConf.padHandle);
		return -1;
	}
	...
	if (ret < 0)
	{
		klog("sceSystemServiceParamGetInt error 0x%08X", ret);
		klog("Failed to obtain ORBIS_SYSTEM_SERVICE_PARAM_ID_ENTER_BUTTON_ASSIGN info!, assigning X as main button.");
		orbisPadConf.crossButtonOK = 1;
	}

	orbispad_initialized=1;
	g_time = timeInMilliseconds();
	klog("ORBISPAD initialized: scePadOpen return handle 0x%X", orbisPadConf.padHandle);
